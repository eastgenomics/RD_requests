...
def find_files_in_project(project: dict, name: str,
    ...
    for vcf in vcfs:
        ...
        file_desc = dxpy.DXFile(vcf["id"]).describe(
        fields={
            "createdBy": True,
                }     
            )
        try:
            job_id = file_desc["createdBy"]["job"]
            job_desc = dxpy.DXJob(job_id).describe(
            fields={
            "input": True,
            "parentAnalysis" : True
                }     
        ...
        try: 
            ci = job_desc["input"]["clinical_indication"]
            if "R149" in ci:
                vcf_to_return.append(vcf)
        except:
            try:
                # try pre dias format
                ci_pre_dias = job_desc["input"]["list_panel_names_genes"]
                if "R149" in ci_pre_dias:
                    vcf_to_return.append(vcf)
            except:
                try:
                    # try dias v1 format of nirvana2vcf inputs
                    analysis_id = job_desc["parentAnalysis"]
                    analysis_desc = dxpy.DXAnalysis(analysis_id).describe(
                    fields={
                    "output": True,
                     }     
                    )
                    file_desc = dxpy.DXFile(analysis_desc["output"]["stage-Fyq5yy0433GXxz691bKyvjPJ.bed_file"]).describe(
                        fields={
                        "name": True,
                            }     
                        )
                    if "R149" in file_desc["name"]:
                        print(file_desc["name"])
                        vcf_to_return.append(vcf)
                except:
                    print(job_desc)
                    print("no inputs containing panel found for sample:", vcf["describe"]["name"])

        

    return vcf_to_return
...
def remove_controls_and_dups(df: pd.DataFrame) -> tuple:
    ...
    print("before control removal and odd pattern removal:", len(df))
    df_no_control = df[mask].copy()
    print("after control removal and odd pattern removal:", len(df_no_control))
    ...
    print("with no controls", len(df_no_control))
    df_no_dups_no_control = df_no_control.drop_duplicates(subset="sample", keep="first")
    print("with no dup: ", len(df_no_dups_no_control))
